...
class SendGridEmailMessageMixin:
    ...
    def prep_message_for_sending(self):
        ...
        # now = tz.localize(datetime.datetime.strptime(timestamp[:26],
        # POSTMARK_DATETIME_STRING)).astimezone(pytz.utc)
        uniqueArgs = {
...
class SendGridEmailMessage(SendGridEmailMessageMixin, EmailMessage):
    ...
    def send(self, *args, **kwargs):
        ...
        logger.debug(
            "Tried to send an email with SendGrid and got response {r}".format(r=response))
...
class SendGridEmailMultiAlternatives(SendGridEmailMessageMixin, EmailMultiAlternatives):
    ...
    def send(self, *args, **kwargs):
        ...
        save_email_message(sender=self, message=self, response=None)
        response = super(
            SendGridEmailMultiAlternatives, self).send(*args, **kwargs)
        logger.debug(
