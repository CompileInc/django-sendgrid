...
POST_EVENTS_RESPONSE_STATUS_CODE = getattr(
    settings, "POST_EVENT_HANDLER_RESPONSE_STATUS_CODE", 200)
...
def handle_single_event_request(request):
    ...
    if timestamp:
        ...
        # this also might need a combined index, but django orm doesn't have
        # this feature yet: https://code.djangoproject.com/ticket/5805
        existingEvents = Event.objects.filter(
            email_message=emailMessage, event_type=event_type, timestamp=event_params["timestamp"])
        unique = existingEvents.count() == 0
    else:
        # no timestamp provided. therefore we cannot enforce any kind of
        # uniqueness
        unique = True
    if unique:
        ...
        event_model = eval(
            EVENT_MODEL_NAMES[event]) if event in EVENT_MODEL_NAMES.keys() else Event
...
def listener(request, statusCode=POST_EVENTS_RESPONSE_STATUS_CODE):
    ...
    if request.method == "POST":
        if request.META["CONTENT_TYPE"].startswith("application/json"):
            ...
            # They contain exactly one JSON string per line, with each line
            ...
            msg = "Unexpected content type: {m}".format(
                m=request.META["CONTENT_TYPE"])
            logger.error(msg)
    else:
        msg = "Request method '{method}' not allowed: {error}".format(
            method=request.method, error=request.method)
        logger.error(msg)
